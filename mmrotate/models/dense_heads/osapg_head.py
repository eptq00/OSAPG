...
from ...core.bbox.assigners.dynamic_assigner import DynamicAssigner
...
class StageRefineRPNHead(OrientedRPNHead):
    ...
    def _init_layers(self):
        """Init layers of a RotatedCRPN stage."""
    ...
    def _anchor_targets_single(self,
        ...
        assign_result = self.assigner.assign(
            flat_anchors, gt_hbboxes, gt_bboxes_ignore,
        ...
        bbox_targets = flat_anchors.new_zeros((flat_anchors.size(0), 6))
        bbox_weights = flat_anchors.new_zeros((flat_anchors.size(0), 6))
        ...
        for img_id in range(len(img_metas)):
            ...
            for i in range(num_levels):
                ...
                bboxes = self.bbox_coder.decode(anchor_list[img_id][i],
                                                bbox_pred, img_shape)
                hbboxes = obb2xyxy(bboxes, version=self.version)
                mlvl_anchors.append(hbboxes)
...
@ROTATED_HEADS.register_module()
class OSAPGHead(BaseDenseHead, BBoxTestMixin):
    def __init__(self, num_stages, stages, train_cfg, test_cfg, init_cfg=None):
        super(OSAPGHead, self).__init__(init_cfg)
    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            cls_score, bbox_pred = stage(x, offset_list)
